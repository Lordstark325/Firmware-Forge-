...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        super.onCreate(state); usb = (UsbManager)getSystemService(USB_SERVICE); buildUi(); scan();
    ...
    private void buildUi() {
        ...
        root.addView(text("FIRMWARE LIBRARY",11,Color.rgb(112,143,130))); firmwareInfo=text("No firmware image selected",13,Color.rgb(190,214,204)); root.addView(firmwareInfo);
        Button choose=button("Choose firmware file"); choose.setOnClickListener(v->{Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.setType("application/octet-stream");i.addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(i,7);}); root.addView(choose, params(-1,56,12));
