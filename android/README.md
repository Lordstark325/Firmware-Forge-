...
Version 0.2 detects USB devices, classifies common ESP serial bridges and modem/router USB interfaces, requests Android USB permission, selects firmware through the Storage Access Framework, and calculates SHA-256 locally. It can also expose an authenticated local control service and USB byte bridge so the paired desktop app can route ESP inspection, backup, flashing, and verification through the phone.
