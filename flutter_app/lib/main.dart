...
class BrandMark extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Text(
      'FF',
      style: TextStyle(color: bg, fontWeight: FontWeight.w900, fontSize: 17),
    ),
  );
}
      ...
      ResponsiveGrid(
        children: [
          const ForgeCard(
            ...
            title: 'USB discovery',
            body:
                'Recognize ESP, Arduino, Pico, STM32, Teensy, Nordic, Adafruit, routers, and serial adapters.',
          ),
          const ForgeCard(
...
class _DevicesPageState extends State<DevicesPage> {
  ...
  Widget build(BuildContext context) => PageFrame(
    ...
    subtitle:
        'Detect common development boards and USB adapters. Unknown devices remain read-only.',
    children: [
      SizedBox(
        ...
        child: ElevatedButton.icon(
          ...
          label: Text(busy ? 'Scanning...' : 'Scan connected devices'),
        ),
      ),
      if (error != null)
        Padding(
      ...
      if (devices.isEmpty)
        const EmptyState(
          ...
          body:
              'Attach a development board through an OTG adapter, approve USB access, then scan again.',
        ),
      ...devices.map(
        (d) => Card(
          child: ListTile(
            ...
            subtitle: Text(
              'VID:${d['vendor']}  PID:${d['product']}  ·  ${d['confidence'] ?? 'candidate'}\n${d['interfaces']} interface(s)',
              style: const TextStyle(color: muted),
            ),
            trailing: Icon(
              d['permission'] == true ? Icons.verified : Icons.lock_outline,
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                StatusPill(
                  active: running,
                  label: running ? 'GATEWAY ONLINE' : 'GATEWAY STOPPED',
                ),
                const SizedBox(height: 18),
                InfoRow(
                  label: 'Phone IP',
                  value: '${info['ip'] ?? 'Connect to Wi-Fi or Tailscale'}',
                ),
                InfoRow(
                  label: 'Control port',
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(
                      running ? 'Stop secure gateway' : 'Start secure gateway',
                    ),
                  ),
                ),
              ],
